...
class CryptowatchExchange extends Exchange {
  constructor(name, label, color, markets) {
    ...
    this.dataSets = markets.map(m => ({ market: m, data: null, expire: null }))
  }
...
const cryptowatchExchanges = [
  new CryptowatchExchange('bitfinex', 'Bitfinex', '#C6FF00', [
    'btcusd',
    'bchusd',
    'bchbtc',
    'ethusd',
  ...
  new CryptowatchExchange('bitstamp', 'Bitstamp', '#00E676', [
    'btcusd',
    'bchusd',
    'bchbtc',
    'ethusd',
  ...
  new CryptowatchExchange('kraken', 'Kraken', '#651FFF', [
    'btcusd',
    'bchusd',
    'bchbtc',
    'ethusd',
  ...
  new CryptowatchExchange('poloniex', 'Poloniex', '#1DE9B6', [
    'btcusdt',
    'bchusdt',
    'bchbtc',
    'ethusdt',
...
export const getters = {
  ...
  getAllExchanges: state => () => [
    ...state.cryptowatchExchanges,
    ...state.quotesExchanges,
  ],
  ...
  getPrice: state => market => state.prices.find(x => x.name === market),
}
...
export const mutations = {
  setExchangeOptions(state, markets) {
    ...
    state.exchangeOptions = allExchanges.filter(e =>
      e.markets.some(m => markets.includes(m)),
    )
  },
...
export const actions = {
  async fetchPrice({ commit }, { exchange, market }) {
    ...
    const data = await this.$axios.$get(
      `price/markets/${exchange}/${market}/1d`,
    )
    commit('updatePrice', { exchange, market, data })
  },
  async fetchCryptowatchExchangeData(
    { getters, commit },
    ...
    const { result: data } = await this.$axios.$get(
      `cryptowatch/markets/${exchange}/${market}/ohlc`,
      {
        params: { convert },
      },
    )
  ...
  async fetchQuotesExchangeData(
    ...
    if (
      !dataSet ||
      (dataSet.buy && dataSet.sell && dataSet.expire < Date.now())
    )
      return
